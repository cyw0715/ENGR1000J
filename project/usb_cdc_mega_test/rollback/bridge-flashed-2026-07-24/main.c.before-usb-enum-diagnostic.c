...
{
    int32_t error = s_last_error;
    uint8_t payload[5] = { s_mega_connected ? 1 : 0, (uint8_t)error,
                            (uint8_t)(error >> 8), (uint8_t)(error >> 16), (uint8_t)(error >> 24) };
...
void app_main(void)
{
    ...
    const cdc_acm_host_driver_config_t cdc_config = {
        ...
        .xCoreID = tskNO_AFFINITY, .new_dev_cb = NULL,
    };
