...
// Faithfully preserved from 111(1).ino (SHA256 1973c42c...f651f8).
...
const uint16_t PRESET_LENGTH_MM[PRESET_COUNT][4] = {
  {164, 101, 147, 138}, {89, 102, 146, 138},
  {200, 80, 176, 177},  {158, 84, 183, 174},
  {161, 96, 87, 140},   {99, 100, 82, 139},
  {222, 93, 147, 190},  {159, 87, 135, 185},
  {168, 107, 147, 83},  {98, 106, 152, 82},
  {229, 92, 204, 134},  {166, 94, 213, 143},
  {161, 96, 92, 82},    {110, 108, 96, 102},
  {229, 96, 152, 134},  {162, 94, 136, 125}
};
const uint16_t REMOTE_TARGET_TOLERANCE_MM = 3;
const uint8_t REMOTE_REQUIRED_CONFIRMATIONS = 2;
const unsigned long PRESET_TIMEOUT_MS = 30000UL;
...
enum RemoteSubmode : uint8_t { REMOTE_MANUAL, REMOTE_PRESET_SELECT };
RemoteSubmode remoteSubmode = REMOTE_MANUAL;
bool presetMovementActive = false;
bool hostRemoteMovementActive = false;
uint8_t activePresetIndex = 0;
uint16_t remoteTargetA[4] = {0, 0, 0, 0};
bool presetReached[4] = {false, false, false, false};
uint8_t presetConfirmationCount[4] = {0, 0, 0, 0};
uint32_t presetProcessedVersion[4] = {0, 0, 0, 0};
...
const int16_t SENSOR_RANGE_MIN_MM = 0;
...
enum MegaState : uint8_t {
  STATE_BOOT, STATE_LOCKED, STATE_REMOTE,
  STATE_ARMED, STATE_ACTIVE, STATE_FAULT, STATE_ESTOP
};
...
bool allSensorsFreshAndInRange() {
  ...
  for (uint8_t a = 0; a < ACTUATOR_COUNT; ++a) {
    if (!sensorValid[a] || now - lastValidMS[a] > SENSOR_STALE_TIMEOUT_MS ||
        currentMM[a] > SENSOR_RANGE_MAX_MM) return false;
  }
  return true;
}

void stopRemoteMovement() {
  hardStopAll();
  presetMovementActive = false;
  hostRemoteMovementActive = false;
}

void driveRemoteLeg(uint8_t a, OutputDirection direction) {
  if (!sensorValid[a] || millis() - lastValidMS[a] > SENSOR_STALE_TIMEOUT_MS) {
    stopLeg(a); return;
  }
  if ((direction == DIR_RETRACT && currentMM[a] <= SENSOR_RANGE_MIN_MM) ||
      (direction == DIR_EXTEND && currentMM[a] >= SENSOR_RANGE_MAX_MM)) {
    stopLeg(a); return;
  }
  if (outputDirA[a] == direction) return;
  if (outputDirA[a] != DIR_IDLE) { stopLeg(a); return; }
  if (millis() - lastStopA[a] >= REVERSAL_DEAD_TIME_MS) driveLeg(a, direction);
}

void startPresetModeRumble() {
  modeRumbleActive = true; modeRumbleStartTime = millis();
}
uint8_t getCurrentRumbleStrength() {
  if (!modeRumbleActive) return 0;
  if (millis() - modeRumbleStartTime >= PRESET_MODE_RUMBLE_DURATION_MS) {
    modeRumbleActive = false; return 0;
  }
  return PRESET_MODE_RUMBLE_STRENGTH;
}
void stopModeRumble() { modeRumbleActive = false; }

void enterPresetSelectionMode() {
  stopRemoteMovement(); remoteSubmode = REMOTE_PRESET_SELECT;
  startPresetModeRumble(); presetSelectionLatched = false;
  Serial.println("REMOTE PRESET SELECT");
}
void enterManualMode() {
  stopRemoteMovement(); remoteSubmode = REMOTE_MANUAL;
  stopModeRumble(); Serial.println("REMOTE MANUAL");
}

void connectPS2() {
  stopRemoteMovement();
  ps2ErrorCode = ps2x.config_gamepad(PS2_CLK, PS2_CMD, PS2_CS, PS2_DAT, false, true);
  Serial.print("PS2 error code: "); Serial.println(ps2ErrorCode);
  if (ps2ErrorCode == 0) {
    ps2Connected = true; ps2ReadFailureCount = 0;
    remoteSubmode = REMOTE_MANUAL; presetSelectionLatched = false;
    activePresetIndex = 0; startToggleArmed = false; previousStartButton = false;
    stopModeRumble();
    Serial.println("PS2 controller connected successfully.");
    Serial.println("Default mode after connection: MANUAL.");
    Serial.println("LEFT PAD DOWN/CROSS -> A1 RETRACT/EXTEND");
    Serial.println("LEFT PAD RIGHT/CIRCLE -> A2 RETRACT/EXTEND");
    Serial.println("LEFT PAD UP/TRIANGLE -> A3 RETRACT/EXTEND");
    Serial.println("LEFT PAD LEFT/SQUARE -> A4 RETRACT/EXTEND");
    Serial.println("START -> Toggle MANUAL / PRESET SELECT mode");
  } else {
    ps2Connected = false; ps2ReadFailureCount = 0;
    remoteSubmode = REMOTE_MANUAL; startToggleArmed = false;
    previousStartButton = false; stopModeRumble();
    Serial.println("PS2 connection failed.");
  }
}

int8_t detectPresetSelection() {
  int leftX = ps2x.Analog(PSS_LX), rightX = ps2x.Analog(PSS_RX);
  ...
  bool any = ps2x.Button(PSB_L2) || ps2x.Button(PSB_L1) ||
    ps2x.Button(PSB_PAD_UP) || ps2x.Button(PSB_PAD_LEFT) ||
    ps2x.Button(PSB_PAD_DOWN) || ps2x.Button(PSB_PAD_RIGHT) ||
    leftStickLeft || leftStickRight || rightStickLeft || rightStickRight ||
    ps2x.Button(PSB_SQUARE) || ps2x.Button(PSB_CROSS) ||
    ps2x.Button(PSB_CIRCLE) || ps2x.Button(PSB_TRIANGLE) ||
    ps2x.Button(PSB_R1) || ps2x.Button(PSB_R2);
  if (!any) { presetSelectionLatched = false; return -1; }
  if (presetSelectionLatched) return -1;
  presetSelectionLatched = true;
  if (ps2x.Button(PSB_L2)) return 0;
  if (ps2x.Button(PSB_L1)) return 1;
  if (ps2x.Button(PSB_PAD_UP)) return 2;
  if (ps2x.Button(PSB_PAD_LEFT)) return 3;
  if (ps2x.Button(PSB_PAD_DOWN)) return 4;
  if (ps2x.Button(PSB_PAD_RIGHT)) return 5;
  if (leftStickLeft) return 6;
  if (leftStickRight) return 7;
  if (rightStickLeft) return 8;
  if (rightStickRight) return 9;
  if (ps2x.Button(PSB_SQUARE)) return 10;
  if (ps2x.Button(PSB_CROSS)) return 11;
  if (ps2x.Button(PSB_CIRCLE)) return 12;
  if (ps2x.Button(PSB_TRIANGLE)) return 13;
  if (ps2x.Button(PSB_R1)) return 14;
  if (ps2x.Button(PSB_R2)) return 15;
  return -1;
}

void startRemoteTarget(const uint16_t targetA[4], bool fromHost, uint8_t presetIndex = 0) {
  if (!allSensorsFreshAndInRange()) { stopRemoteMovement(); Serial.println("ERR REMOTE_SENSOR_GATE"); return; }
  for (uint8_t a = 0; a < 4; ++a) {
    if (targetA[a] > SENSOR_RANGE_MAX_MM) {
      stopRemoteMovement(); Serial.println("ERR REMOTE_ABSOLUTE_RANGE"); return;
    }
  }
  hardStopAll();
  for (uint8_t a = 0; a < 4; ++a) {
    remoteTargetA[a] = targetA[a]; presetReached[a] = false;
    presetConfirmationCount[a] = 0; presetProcessedVersion[a] = measurementVersion[a];
  }
  activePresetIndex = presetIndex; presetStartTime = millis();
  hostRemoteMovementActive = fromHost; presetMovementActive = !fromHost;
  Serial.print(fromHost ? "REMOTE HOST TARGET" : "REMOTE PRESET TARGET");
  Serial.print(" A1/A2/A3/A4=");
  for (uint8_t a = 0; a < 4; ++a) { if (a) Serial.print(','); Serial.print(remoteTargetA[a]); }
  Serial.println();
}

void updateRemoteTargetMovement() {
  if (!presetMovementActive && !hostRemoteMovementActive) return;
  if (millis() - presetStartTime >= PRESET_TIMEOUT_MS) {
    stopRemoteMovement(); Serial.println("ERR REMOTE_TIMEOUT ALL_LOW"); return;
  }
  if (!allSensorsFreshAndInRange()) {
    stopRemoteMovement(); Serial.println("ERR REMOTE_SENSOR_INVALID ALL_LOW"); return;
  }
  bool allReached = true;
  for (uint8_t a = 0; a < 4; ++a) {
    int difference = (int)remoteTargetA[a] - (int)currentMM[a];
    if (abs(difference) <= REMOTE_TARGET_TOLERANCE_MM) {
      stopLeg(a);
      if (measurementVersion[a] != presetProcessedVersion[a]) {
        presetProcessedVersion[a] = measurementVersion[a];
        if (presetConfirmationCount[a] < 255) presetConfirmationCount[a]++;
      }
      presetReached[a] = presetConfirmationCount[a] >= REMOTE_REQUIRED_CONFIRMATIONS;
      if (!presetReached[a]) allReached = false;
    } else {
      presetReached[a] = false; presetConfirmationCount[a] = 0;
      presetProcessedVersion[a] = measurementVersion[a]; allReached = false;
      driveRemoteLeg(a, difference > 0 ? DIR_EXTEND : DIR_RETRACT);
    }
  }
  if (allReached) {
    stopRemoteMovement(); Serial.println("OK REMOTE_TARGET_REACHED ALL_LOW");
  }
}

void updateRemotePS2() {
  if (state != STATE_REMOTE || hostRemoteMovementActive) return;
  if (!ps2Connected) {
    stopRemoteMovement(); remoteSubmode = REMOTE_MANUAL;
    startToggleArmed = false; previousStartButton = false; stopModeRumble();
    if (millis() - lastReconnectAttempt >= RECONNECT_INTERVAL_MS) {
      lastReconnectAttempt = millis(); connectPS2();
    }
    return;
  }
  bool readOK = ps2x.read_gamepad(false, getCurrentRumbleStrength());
  if (!readOK) {
    if (ps2ReadFailureCount < 255) ps2ReadFailureCount++;
    if (ps2ReadFailureCount >= PS2_MAX_READ_FAILURES) {
      stopRemoteMovement(); ps2Connected = false; remoteSubmode = REMOTE_MANUAL;
      startToggleArmed = false; previousStartButton = false;
      stopModeRumble(); lastReconnectAttempt = millis();
      Serial.println("PS2 disconnected; ALL_LOW");
    }
    return;
  }
  ps2ReadFailureCount = 0;
  bool startNow = ps2x.Button(PSB_START);
  if (!startToggleArmed) {
    previousStartButton = startNow;
    if (!startNow) { startToggleArmed = true; previousStartButton = false; }
  } else {
    bool edge = startNow && !previousStartButton; previousStartButton = startNow;
    if (edge) {
      if (remoteSubmode == REMOTE_MANUAL) enterPresetSelectionMode();
      else enterManualMode();
      return;
    }
  }
  if (remoteSubmode == REMOTE_MANUAL) {
    presetMovementActive = false;
    hostRemoteMovementActive = false;
    bool extend[4] = {ps2x.Button(PSB_CROSS), ps2x.Button(PSB_CIRCLE),
                      ps2x.Button(PSB_TRIANGLE), ps2x.Button(PSB_SQUARE)};
    bool retract[4] = {ps2x.Button(PSB_PAD_DOWN), ps2x.Button(PSB_PAD_RIGHT),
                       ps2x.Button(PSB_PAD_UP), ps2x.Button(PSB_PAD_LEFT)};
    for (uint8_t a = 0; a < 4; ++a) {
      if (extend[a] && !retract[a]) driveRemoteLeg(a, DIR_EXTEND);
      else if (retract[a] && !extend[a]) driveRemoteLeg(a, DIR_RETRACT);
      else stopLeg(a);
    }
  } else {
    int8_t selected = detectPresetSelection();
    if (selected >= 0) startRemoteTarget(PRESET_LENGTH_MM[selected], false, (uint8_t)selected);
    updateRemoteTargetMovement();
  }
}

void enterLocked() {
  stopRemoteMovement(); hardStopAll(); state = STATE_LOCKED; lastCommandSeq = 0;
  for (uint8_t i = 0; i < ACTUATOR_COUNT; ++i) {
    targetDelta[i] = 0; appliedDelta[i] = 0;
  }
}
void enterRemote() {
  stopRemoteMovement(); hardStopAll(); state = STATE_REMOTE; lastCommandSeq = 0;
  remoteSubmode = REMOTE_MANUAL; startToggleArmed = false; previousStartButton = false;
  for (uint8_t i = 0; i < ACTUATOR_COUNT; ++i) {
    targetDelta[i] = 0; appliedDelta[i] = 0;
  }
}
void enterDisarmed() { enterLocked(); }
...
void updateAppliedDeltas() {
  if (state == STATE_BOOT || state == STATE_LOCKED || state == STATE_REMOTE ||
      state == STATE_FAULT || state == STATE_ESTOP) {
...
const char *stateName() {
  switch (state) {
    case STATE_BOOT: return "BOOT"; case STATE_LOCKED: return "LOCKED";
    case STATE_REMOTE: return remoteSubmode == REMOTE_MANUAL ? "REMOTE_MANUAL" : "REMOTE_PRESET";
...
void printStatus() {
  ...
  Serial.print(" remote_active="); Serial.print((presetMovementActive || hostRemoteMovementActive) ? 1 : 0);
  Serial.print(" remote_source=");
  Serial.print(hostRemoteMovementActive ? "console" : (presetMovementActive ? "ps2" : "none"));
  Serial.print(" remote_preset="); Serial.print(presetMovementActive ? (int)activePresetIndex : -1);
  Serial.print(" remote_target=");
  for (uint8_t a = 0; a < 4; ++a) { if (a) Serial.print(","); Serial.print(remoteTargetA[a]); }
  Serial.print(" ps2_connected="); Serial.print(ps2Connected ? 1 : 0);
  Serial.print(" remote_submode="); Serial.print(remoteSubmode == REMOTE_MANUAL ? "manual" : "preset");
  Serial.println();
}
...
void processCommand(char *command) {
  ...
  if (strcmp(command, "STOP") == 0) {
    ...
    enterLocked(); Serial.println("OK STOPPED LOCKED ALL_LOW"); return;
  }
  if (state == STATE_FAULT) { Serial.println("ERR FAULT_LATCHED_RESET_REQUIRED"); return; }

  if (strcmp(command, "ARM REMOTE") == 0) {
    if (state != STATE_LOCKED) { Serial.println("ERR OWNER_NOT_LOCKED"); return; }
    if (!allSensorsFreshAndInRange()) { Serial.println("ERR ARM_SENSOR_GATE"); return; }
    enterRemote(); lastHeartbeatMS = millis();
    Serial.println("OK ARMED_REMOTE PS2_ENABLED"); return;
  }
  ...
  if (state != STATE_REMOTE && state != STATE_ARMED && state != STATE_ACTIVE) {
    Serial.println("ERR NOT_ARMED"); return;
  }
  ...
  if (strcmp(kind, "SET_REMOTE") == 0) {
    if (state != STATE_REMOTE) { Serial.println("ERR OWNER_NOT_REMOTE"); return; }
    uint16_t solverTarget[4];
    for (uint8_t i = 0; i < 4; ++i) {
      int16_t value = 0;
      if (!parseInt16(strtok_r(NULL, " ", &save), value) ||
          value < SENSOR_RANGE_MIN_MM || value > SENSOR_RANGE_MAX_MM) {
        Serial.println("ERR SET_REMOTE_FORMAT_OR_RANGE"); return;
      }
      solverTarget[i] = (uint16_t)value;
    }
    if (strtok_r(NULL, " ", &save) != NULL) { Serial.println("ERR SET_REMOTE_EXTRA"); return; }
    uint16_t targetA[4];
    for (uint8_t solver = 0; solver < 4; ++solver) {
      targetA[SOLVER_TO_MEGA[solver]] = solverTarget[solver];
    }
    lastCommandSeq = seq; lastHeartbeatMS = millis();
    startRemoteTarget(targetA, true); Serial.println("OK SET_REMOTE"); return;
  }
  if (strcmp(kind, "SET_DELTA") == 0) {
    if (state != STATE_ARMED && state != STATE_ACTIVE) {
      Serial.println("ERR OWNER_NOT_IMU"); return;
    }
    int16_t candidate[4];
    for (uint8_t i = 0; i < 4; ++i) {
...
void updateRemoteOwner() {
  if (state != STATE_REMOTE) return;
  if (millis() - lastHeartbeatMS > HEARTBEAT_TIMEOUT_MS) {
    stopRemoteMovement(); enterLocked();
    strncpy(faultReason, "none", sizeof(faultReason));
    Serial.println("REMOTE HEARTBEAT TIMEOUT LOCKED ALL_LOW");
    return;
  }
  if (hostRemoteMovementActive) updateRemoteTargetMovement();
  else updateRemotePS2();
}
...
void setup() {
  ...
  hardStopAll(); delay(1500); Serial.println("MEGA_LANDER_HYBRID_V1 BOOT LOCKED ALL_LOW");
  ...
  enterLocked(); Serial.println("READY HYBRID_V1 LOCKED PS2_AVAILABLE_AFTER_ARM_REMOTE");
  connectPS2();
}
...
void loop() {
  ...
  updateRemoteOwner();
  updateControl();
  if (state == STATE_LOCKED || state == STATE_FAULT || state == STATE_ESTOP) hardStopAll();
